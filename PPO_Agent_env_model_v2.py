...
class PPO(tf.keras.Model):
    ...
    def process_training(self,obs, next_obs, actions, returns, advantages, old_log_policy):
        ...
        with tf.GradientTape() as tape:
            ...
            old_log_policy = tf.reshape(old_log_policy, (len(actions),-1))
            ...
            # print((prob_ratio * advantages).shape)
            # print(( weighted_clipped_probs * advantages).shape)
    ...
    def train(self, next_state_value, epoch):
        ...
        returns, advantages = self.compute_advantages(rewards, state_values, dones)
        ...
        advantages= tf.reshape(tf.cast(tf.convert_to_tensor(advantages), dtype = tf.float32), (len(advantages),1))
        returns= tf.reshape(tf.cast(tf.convert_to_tensor(returns), dtype = tf.float32), (len(returns),1))
