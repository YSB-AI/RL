...
class ContinuousA3CAgent_Optimization(tf.keras.Model):
    ...
    def Compute_loss(self, obs):
        with tf.GradientTape() as tape:
            ...
            if self.use_GAE:
                advantage = self.GAE_implementation(target_state_values, state_values, rewards, done)
            else:
        ...
        if self.use_LSTM:
            return rewards*self.reward_scaler, done,  entropy,actor_loss,critic_loss,loss, advantage,target_state_values,state_values ,actions, original_new_obs
        else:
            return rewards*self.reward_scaler, done,  entropy,actor_loss,critic_loss,loss, advantage,target_state_values,state_values ,actions, new_obs

    def run_and_train_agent(self, epoch, obs):

        rewards, done,  entropy,actor_loss,critic_loss,loss, advantage,target_state_values,state_values ,actions, new_obs = self.Compute_loss(obs)
    ...
    def evaluate(self, eval_env, n_tries=1, hyp= False):
        ...
        for _ in range(n_tries):
            ...
            while(True):
                ...
                action, _,mu, sigma= self.actorcritic_agent(obs, inference = True)
                if self.environment_name in ["BipedalWalker-v3"]:
                    action = action[0,:]
...
class A3CAgent_Optimization(tf.keras.Model):
    ...
    def Compute_loss(self, obs):
        ...
        if self.use_LSTM:
            return rewards*self.reward_scaler, done, random_choice, entropy,actor_loss,critic_loss,loss, advantage,target_state_values,state_values ,actions, original_new_obs
        else:
            return rewards*self.reward_scaler, done, random_choice, entropy,actor_loss,critic_loss,loss, advantage,target_state_values,state_values ,actions, new_obs
        
        
    def run_and_train_agent(self, epoch, obs):

        rewards, done, random_choice, entropy,actor_loss,critic_loss,loss, advantage,target_state_values,state_values ,actions, new_obs=  self.Compute_loss(obs)
        self.total_rewards = self.total_rewards + rewards[0]
    ...
    for k in range(n_tries):
        ...
        while(True):
            ...
            if continuous_action_space:
                action, state_values, mu, sigma = eval_model.actorcritic_agent(obs, inference = True)
            else: 
