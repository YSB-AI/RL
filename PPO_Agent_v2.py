...
class PPO(tf.keras.Model):
    ...
    def train(self, next_state_value):
        ...
        for k in range(self.training_epoch):  
            actor_loss, critic_loss, state_value, entropy, prob_ratio = self.process_training(obs, action_arr, returns, advantages, old_log_probs)
            # with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
    ...
    def run_agent(self, epoch, obs):
        ...
        if done or len(self.memory)== self.memory_size:        
            ...
            actor_loss, critic_loss, advantages, state_values, returns, entropy, prob_ratio = self.train(next_state_value)
            self.memory.clear()
...
def run_training(training_steps,   discount,  dense_units_act,  dense_units_crit,num_layer_a,num_layer_c,
                 ...
                 lr_actor= 0.001, lr_critic= 0.001,   gae_lambda=0.95, training_epoch= 20, entropy_coeff= 0.01, policy_clip = 0.2,memory_size= 50, id = 1):
    ...
    with tqdm.trange(training_steps) as t:
        ...
        for epoch in t:
            ...
            if epoch %save_factor == 0: 
                model.save_weights('./checkpoints/PPOagent'+id)
