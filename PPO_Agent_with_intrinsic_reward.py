...
class PPO(tf.keras.Model):
    ...
    evaluation_epoch = 2500, environment_name="", reward_scaler  = 1, gae_lambda=0.95, policy_clip = 0.2, training_epoch=20, entropy_coeff = 0.05, memory_size = 50, scaling_factor_reward = 0.1):
    ...
    def calculate_intrinsic_reward(self, obs, action, next_obs):
        # Example function to calculate intrinsic reward based on prediction error
    ...
    def process_training(self,obs, next_obs, action_arr, returns, advantages, old_log_probs):
        ...
        with tf.GradientTape() as tape1, tf.GradientTape() as tape2, tf.GradientTape() as tape3:
            ...
            predicted_next_obs = self.env_model(obs,
                                                tf.reshape(action_arr, (len(action_arr),self.env.action_space.shape[0])), 
                                                )
    ...
    def train(self, next_state_value):
        ...
        for k in range(self.training_epoch):  
            actor_loss, critic_loss, state_value, entropy, prob_ratio, model_loss = self.process_training(obs, next_obs, action_arr, returns, advantages, old_log_probs)
    ...
    def run_agent(self, epoch, obs):
        ...
        intrinsic_reward = self.calculate_intrinsic_reward( obs.reshape((1,self.obs_shape[0])),
...
def run_training(training_steps,   discount,  dense_units_act,  dense_units_crit,dense_units_model,num_layer_a,num_layer_c,num_layer_m,
                 ...
                 policy_clip = 0.2,memory_size= 50, id = 1, scaling_factor_reward = 0.1):
