...
class ContinuousA3CAgent_Optimization(tf.keras.Model):
    ...
    evaluation_epoch = 2500, environment_name="MountainCar-v0", num_layers =1,  sigma_noise= 1e-5, reward_scaler  = 1, use_GAE = False):
        ...
        with self.tb_summary_writer.as_default():
            ...
            tf.summary.scalar('num_layers', num_layers, step = 0)
            for i in range(num_layers):
                tf.summary.scalar('dense_units_'+str(i), dense_units[i], step=0)
        ...
        if self.end_of_episode == -1 :
            self.train_envs_list = [gym.make(environment_name) for _ in range(n_enviroment)]
        else:
    ...
    def env_step(self, actions):
        ...
        for i in range(self.n_enviroment):
            ...
            if self.iter[i] == self.end_of_episode or  done[i][0] == 1:
                if self.PRINT == True :  print(f"Env {i} | Iteration {self.iter[i]} | ended the episode with done = {done[i][0]}")
    ...
    def Compute_loss(self, obs, lstm_state):
        with tf.GradientTape() as tape:
            
          
            actions, state_values, mu, sigma,lstm_state = self.actorcritic_agent(obs, lstm_state = lstm_state)
            ...
            _, next_state_values, _, _ , _= self.actorcritic_agent(new_obs, lstm_state = lstm_state)

            target_state_values = rewards + self.discount*next_state_values*done
            ...
            if self.environment_name in ["BipedalWalker-v3"]:
                ...
                logp_actions = tf.reduce_sum(normal_dist.log_prob(tf.stop_gradient(actions)), axis=-1)
                ...
                actor_loss = -tf.reduce_mean(normal_dist.log_prob(tf.stop_gradient(actions)) * tf.stop_gradient(advantage)) -  self.entropy_factor * tf.reduce_mean(entropy, axis=0)
        ...
        return rewards*self.reward_scaler, done,  entropy,actor_loss,critic_loss,loss, advantage,target_state_values,state_values ,actions, new_obs, lstm_state

    def run_and_train_agent(self, epoch, obs, lstm_state):

        rewards, done,  entropy,actor_loss,critic_loss,loss, advantage,target_state_values,state_values ,actions, new_obs, lstm_state = self.Compute_loss(obs, lstm_state)
        ...
        if (done[0] == 0) and epoch >0  :
            if self.PRINT == True : print(f"Epoch {epoch} ended the episode : {1-done[0]}")
    ...
    def evaluate(self, eval_env, n_tries=1, hyp= False):
        ...
        for _ in range(n_tries):
            ...
            while(True):
                ...
                action, _,mu, sigma,lstm_state= self.actorcritic_agent(obs, lstm_state= lstm_state, inference = True)
                if self.environment_name in ["BipedalWalker-v3"]:
                    action = action[0,:]
...
class ContinuousActorCriticAgent(tf.keras.Model):
    def __init__(self,  env,  d, lstm_unit,  num_layers = 1, use_LSTM=False, sigma_noise = 1e-5):
        super(ContinuousActorCriticAgent, self).__init__()
        self.env = env
        self.d_state_value = Dense(1, activation='linear', name ="v")
        self.use_LSTM = use_LSTM
        self.lstm = LSTM(lstm_unit, return_sequences=False, return_state = True)
        ...
        self.dense_layers = [Dense(d[i], activation='relu', name ="inp_"+str(i)) for i in range(num_layers)]

    def call(self, observations,  lstm_state = None, inference = False):
        
        observations = tf.expand_dims(observations, axis = 1)
        if lstm_state is None:
            x, h_state, c_state = self.lstm(observations)
        else:
            x, h_state, c_state = self.lstm(observations, initial_state= lstm_state)
        ...
        mu, sigma = tf.squeeze(mu*self.env.action_space.high[0]), tf.squeeze(sigma + self.sigma_noise)
        ...
        if inference:
            return action, state_value[:,0], mu, sigma, lstm_state
        return action[0,:], state_value[:,0], mu, sigma, lstm_state
...
class ActorCriticAgent(tf.keras.Model):
    ...
    def call(self, observations, lstm_state= None):
        ...
        return logits, state_value[:,0], lstm_state
...
class A3CAgent_Optimization(tf.keras.Model):
    ...
    evaluation_epoch = 2500, environment_name="MountainCar-v0" , reward_scaler = 1, use_GAE = False):
    ...
    def Compute_loss(self, obs, lstm_state):
        with tf.GradientTape() as tape:
            ...
            logits, state_values, lstm_state= self.actorcritic_agent(obs, lstm_state = lstm_state)
            ...
            _, next_state_values,_ = self.actorcritic_agent(new_obs, lstm_state = lstm_state)
        ...
        return rewards*self.reward_scaler, done, random_choice, entropy,actor_loss,critic_loss,loss, advantage,target_state_values,state_values ,actions, new_obs, lstm_state
        
        
    def run_and_train_agent(self, epoch, obs, lstm_state):

        rewards, done, random_choice, entropy,actor_loss,critic_loss,loss, advantage,target_state_values,state_values ,actions, new_obs, lstm_state=  self.Compute_loss(obs, lstm_state)
        self.total_rewards = self.total_rewards + rewards[0]
    ...
    def evaluate(self, eval_env, n_tries=1, hyp= False):
        ...
        for _ in range(n_tries):
            ...
            while(True):
                ...
                obs = obs.reshape((1,self.obs_shape[0]))
                logit, _, lstm_state= self.actorcritic_agent(obs, lstm_state)
...
class MyHyperModel(kt.HyperModel):
    def __init__(self, hyper_dir, writer = "logs_hyper/A3C/" , use_LSTM=False, exploration_tech=None,
                  ...
                  continuous_actions_space = False , reward_scaler = 1, use_GAE= False,
                  n_dense_layers= 1):
    ...
    def build(self, hp):
        ...
        discount = discount = hp.Float('discount',self.discount_min, self.discount_max, step =0.01)
        ...
        if self.continuous_actions_space:
            ...
            if self.n_dense_layers>1 :
                num_layers = hp.Int('n_dense_layers', 1, self.n_dense_layers)
            else : 
                num_layers = self.n_dense_layers
            dense_units =  []
            for i in range(num_layers):
                dense_name = 'dense_units_'+str(i)
                d_n  = hp.Int(dense_name, self.dense_min, self.dense_max) 
                dense_units.append(d_n)
                time.sleep(1)
    ...
    def fit(self, hp, model, x, y,  callbacks=None, **kwargs):
        ...
        for epoch in range(training_steps):
            ...
            new_obs, lstm_state = model.run_and_train_agent(epoch, obs, lstm_state)
...
def run_training(training_steps, learning_rate, entropy_factor, exploration_tech, discount, time_to_update, dense_units, lstm_units, 
                 ...
                 environment_name="MountainCar-v0", reward_scaler = 1, continuous_space_actions = False, sigma_noise = 1e-5, return_agent = False, use_GAE = False, agent=None):
...
def final_evaluation(eval_model, eval_env, n_tries=1, exploration ="soft", video_name = "./A3C_soft_video.mp4", sucess_criteria_epochs= 100,
                      reward_scaler = 1, continuous_action_space = False):
    rewards_history = []
    ...
    for k in range(n_tries):
        ...
        while(True):
            ...
            if continuous_action_space:
                action, _,mu, sigma,lstm_state= eval_model.actorcritic_agent(obs, lstm_state= lstm_state, inference = True)
            else: 
            ...
            if eval_model.environment_name in ["BipedalWalker-v3"]:
                action = action[0]
                obs, reward, terminated, truncated , info = eval_env.step(action)
            else:
                obs, reward, terminated, truncated , info = eval_env.step(action)
